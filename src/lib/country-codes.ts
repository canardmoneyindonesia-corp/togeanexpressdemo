...
// Ordered by Togean Islands' top visitor markets (domestic first, then the
// European dive-tourism crowd, then other frequent nationalities). These are
// surfaced at the top of the country-code picker.
export const POPULAR_ISO = [
  "ID", "DE", "FR", "NL", "IT", "CH", "GB", "BE", "AU", "US", "ES", "AT", "CZ", "PL", "CA",
];

// Complete ISO 3166-1 list with E.164 calling codes.
...
/** Popular countries (in market order) followed by the rest, alphabetical. */
export function orderedCountries(): { popular: Country[]; rest: Country[] } {
  const popular = POPULAR_ISO.map((iso) => byIso.get(iso)!).filter(Boolean);
  const popularSet = new Set(POPULAR_ISO);
  const rest = COUNTRIES.filter((c) => !popularSet.has(c.iso)).sort((a, b) =>
